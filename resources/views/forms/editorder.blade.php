...
<body>
    <div id="app">
        ...
        <main class="container">
            ...
            <br>
            <form action="{{ route('order.update', ['id' => $orderData->id]) }}" method="POST" id="mainForm" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select id="searchDropdown" class="form-control" name="selected_value" required>
                            ...
                            <option value="{{ $vehicles->id }}" {{ $vehicles->id == $orderData->vehicle_id ? 'selected' : "" }} >{{ $vehicles->Model }} | {{ $vehicles->Type }} | Rp.{{$vehicles->Price}}</option>
                            @empty
